    ...
    <section id="about" className="py-20 bg-white border-b border-slate-100">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-16">
        ...
        <div className="text-center max-w-3xl mx-auto space-y-4">
          <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-[#1B4F72]/10 text-[#1B4F72] text-xs font-bold uppercase tracking-wider">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 items-start">
          ...
          <div className="lg:col-span-7 space-y-8">
            <div className="prose prose-slate max-w-none space-y-4">
              <h3 className="text-2xl font-bold text-slate-900 flex items-center gap-2">
                <BookCheck className="w-6 h-6 text-[#1B4F72]" />
            ...
            <div className="space-y-4 bg-slate-50/80 p-6 rounded-2xl border border-slate-200/80">
              <h4 className="text-lg font-bold text-[#1B4F72] flex items-center gap-2">
                <Award className="w-5 h-5 text-[#2874A6]" />
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                {CHAPTERS.map((chap) => (
                  <div key={chap.id} className="flex items-start gap-3 bg-white p-3 rounded-xl border border-slate-200/60 shadow-2xs">
                    <span className="shrink-0 w-7 h-7 rounded-lg bg-[#1B4F72] text-white font-bold text-xs flex items-center justify-center">
                      {chap.id}
                    </span>
                    <div>
                      <span className="text-xs font-bold text-[#1B4F72] block">{chap.number}</span>
          ...
          <div className="lg:col-span-5 space-y-6">
            ...
            {/* Book Meta Box */}
            <div className="bg-[#1B4F72] text-white rounded-2xl p-6 sm:p-8 shadow-xl space-y-6 relative overflow-hidden">
              <div className="absolute -right-10 -bottom-10 opacity-10 text-9xl font-black">
                ⚛
              </div>

              <h3 className="text-xl font-extrabold border-b border-white/15 pb-4 text-sky-100 flex items-center gap-2">
              ...
              <div className="space-y-4 text-sm">
                <div>
                  <span className="text-xs text-sky-200 uppercase font-semibold block">Атауы</span>
                  <p className="font-bold text-white text-base mt-0.5">{BOOK_METADATA.title}</p>
                </div>

                <div>
                  <span className="text-xs text-sky-200 uppercase font-semibold block">Университет / Мекеме</span>
                  <p className="font-medium text-sky-50 mt-0.5">{BOOK_METADATA.university}</p>
                </div>

                <div className="grid grid-cols-2 gap-4 pt-2 border-t border-white/10">
                  <div>
                    <span className="text-xs text-sky-200 uppercase font-semibold block">Авторлары</span>
                    <p className="font-semibold text-white mt-0.5">{BOOK_METADATA.authors}</p>
                  </div>
                  <div>
                    <span className="text-xs text-sky-200 uppercase font-semibold block">Оқыту бағыты</span>
